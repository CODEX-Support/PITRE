...
class Server:
    def __init__(self):
        self.uri = "PYRO:example.pitd@localhost:9093"
        self.pitd_server = Pyro4.Proxy(self.uri)
    #authenticate user 
    ...
    def tfn_verify(self,tfn_data):
        is_verified = self.pitd_server.tfn_verification(tfn_data)
        return is_verified
    
    #calculate tax return estimate if user has TFN
    def calculate_tax_return_estimate_for_TFN(self,tfn):
        estimate_results = self.pitd_server.get_estimate_result(tfn)
        print("Requesting estimate for the TFN Number")
